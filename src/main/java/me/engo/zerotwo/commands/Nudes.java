...
public class Nudes extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent Context) {
        ...
        if (messageSent[0].equalsIgnoreCase(c.prefix + "nudes")) {
                ...
                if (messageSent.length < 3) {
                    ...
                    if (c.nsfw) {
                        if (Context.getChannel().isNSFW()) {
                        	try {
                                URL url = new URL("https://nekobot.xyz/api/image?type=4k");
                                JSONTokener tokener = new JSONTokener(url.openStream());
                                JSONObject obj = new JSONObject(tokener);
                                String message = obj.getString("message");

                                EmbedBuilder em = new EmbedBuilder();
                                em.setImage("https://nekobot.xyz/4k/" + message);
